...
| **5** | `economically_unjustified` | $47.00 | `NO_ACTION` | `NO_ACTION` | Net recovery $\le \$0$; graph halts at Economic Gate (Capital Preserved). |
...
2. **Minute 2: The Visual Decision Machine (`/engine`)**
   ...
   * Click **▶ Replay Decision**. Watch the case token travel through Event Gate $\rightarrow$ Revenue Truth $\rightarrow$ Diagnosis $\rightarrow$ Action Arena $\rightarrow$ Economic Gate $\rightarrow$ Policy Gate $\rightarrow$ Execution Dispatch $\rightarrow$ Verification.
