...
| `notify_customer` | No | N/A | $0.10 | $0.00 | ≤ $0.00 | Ineligible Context |

### **Execution & Outcome**:
1. Net Recovery = **`+$186.25`** > $0.00 $\rightarrow$ Decision: `attempt_capture_retry`.
2. Policy Gate checks: Context v1 current, Autonomous budget approved $\rightarrow$ Status: `APPROVED`.
3. Execution Dispatch: Calls Razorpay capture adapter $\rightarrow$ Status: `VERIFYING` (Risk remains `$249.00`).
4. Gateway Reconciliation: Webhook arrives with `payment_state = captured` $\rightarrow$ Status: **`RECOVERED`**, Current Risk = **`$0.00`**.
...
| Candidate Action | Eligible | Success Prob ($P$) | Cost ($C$) | Gross Expected | Expected Net Recovery | Status |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| `attempt_capture_retry` | No | 65.0% | $50.00 | $30.55 | -$19.45 | Ineligible / Net $\le 0$ |
| `manual_review` | Yes | 40.0% | $25.00 | $18.80 | -$6.20 | Net $\le 0$ |
